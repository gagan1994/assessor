...
import com.assessor.android.cam.CommonUtils;
...
public class PictureCapturingServiceImplNew extends APictureCapturingService {

    private static final String TAG = PictureCapturingServiceImplNew.class.getSimpleName();
     ...
     * 最大允许的拍照尺寸（像素数）
     */
    long mMaxPicturePixels = 3840 * 2160;
    ...
    CameraDevice.StateCallback mStateCallback = new CameraDevice.StateCallback() {
        ...
        public void onError(@NonNull CameraDevice cameraDevice, int error) {
            ...
            cameraDevice.close();
            mCameraDevice = null;
            Toast.makeText(context, "Camera failed to turn on, try again", Toast.LENGTH_LONG).show();
            mFinishCalled = true;
     ...
     */
    PictureCapturingServiceImplNew(Activity activity) {
        super(activity);
    }
    ...
    void setUpCameraOutputs(int width, int height) {
        ...
        try {
            for (String cameraId : manager.getCameraIdList()) {
                ...
                // coordinate.
                int displayRotation = Surface.ROTATION_0;//getWindowManager().getDefaultDisplay().getRotation();
                ...
                int rotatedPreviewWidth = largest.getWidth();
                int rotatedPreviewHeight = largest.getHeight();
    ...
    void createCameraPreviewSession() {
        try {
            // SurfaceTexture texture = new SurfaceTexture();// mTextureView.getSurfaceTexture();
            //assert texture != null;
            ...
            outputSurfaces1.add(mImageReader.getSurface());
            // Here, we create a CameraCaptureSession for camera preview.
            mCameraDevice.createCaptureSession(outputSurfaces1, new CameraCaptureSession.StateCallback() {
                @Override
    ...
    public void startCapturing(PictureCapturingListener listener) {
        mFile = CommonUtils.createImageFile(context, System.currentTimeMillis() + ".jpeg");
        mFile2 = CommonUtils.createImageFile(context, System.currentTimeMillis() + ".jpeg");
    ...
    public static APictureCapturingService getInstance(final Activity activity) {
        return new PictureCapturingServiceImplNew(activity);
    }
        ...
        public void run() {
            ...
            try {
                ...
                mHandler.post(() -> {
                    ...
                    mFinishCalled = true;
                    Toast.makeText(context, mFile.toString(), Toast.LENGTH_LONG).show();
                    //finish();
                });
                mFinishCalled = true;
            } catch (Exception e) {
