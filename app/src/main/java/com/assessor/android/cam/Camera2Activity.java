...
public class Camera2Activity {
    ...
    public static Camera2Activity newInstance(Activity context) {
        return new Camera2Activity(context);
    }

    Camera2Activity(Activity context) {
    ...
    CameraDevice.StateCallback mStateCallback = new CameraDevice.StateCallback() {
        ...
        public void onError(@NonNull CameraDevice cameraDevice, int error) {
            ...
            cameraDevice.close();
            mCameraDevice = null;
            //Toast.makeText(TAG, "Camera failed to turn on, try again", Toast.LENGTH_LONG).show();
            mFinishCalled = true;
...
//            Surface surface = new Surface(texture);
    ...
    class ImageSaver implements Runnable {
        ...
        public void run() {
            ...
            buffer.get(bytes);
            try {
                if (mFileName.exists()) mFileName.delete();
                FileOutputStream output = new FileOutputStream(mFileName);
                output.write(bytes);
                try {
                    mImage.close();
...
//                });
    ...
    public void startCapturing(PictureCapturingListener listener) {
        mFile = CommonUtils.createImageFile(mContext, "cam1" + System.currentTimeMillis() + ".jpeg");
        mFile1 = CommonUtils.createImageFile(mContext, "cam2" + System.currentTimeMillis() + ".jpeg");
