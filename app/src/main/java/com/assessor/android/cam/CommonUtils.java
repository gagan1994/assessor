...
public class CommonUtils {
    ...
    public static int getNavigationBarHeightInPx(Context context) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.JELLY_BEAN_MR1) return dp2px(context, 48);
        int navBarHeightInPx = 0;
        Resources rs = context.getResources();
        int id = rs.getIdentifier("navigation_bar_height", "dimen", "android");
        if (id > 0 && hasNavigationBar(context)) navBarHeightInPx = rs.getDimensionPixelSize(id);
...
// Not sure if the / is on the path or not
        File dir = new File(baseDir + File.separator + "Assessor");

        //File dir = new File(context.getCacheDir(), "assessor");
